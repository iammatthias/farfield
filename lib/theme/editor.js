...
//
// Layer 2 — the document editor: when the page carries a [data-doc-host]
// element, the script mounts a writing surface there, in the page itself —
// the page IS the editor, with no dialog and no mode to enter. With FARFIELD.editdoc set (the host's session-gated
...
(function () {
  ...
  // opts: { accept, label } — defaults to images only (series are galleries).
  function uploadZone(onDone, opts) {
    opts = opts || {};
    var idle = opts.label || "Drop an image here, or click to upload";
    var label = el("span", { class: "embed-drop-text", text: idle });
    var input = el("input", { type: "file", accept: opts.accept || "image/*" });
  ...
  function blobGrid(opts) {
    ...
    // mediaCell renders a grid tile for one blob meta, by mime: images use
    // their generated thumbnail, videos show a first frame, audio and other
    // files get a glyph. An unknown mime (a cid-only prepend after an image
    // upload) falls through to the image branch.
    function mediaCell(b) {
      var m = b.mime || "";
      if (/^video\//.test(m))
        return el("video", { src: blobURL(b.cid) + "#t=0.1",
          preload: "metadata", muted: "", playsinline: "" });
      if (/^audio\//.test(m)) return el("span", { class: "embed-glyph", text: "♪" });
      if (m && !/^image\//.test(m)) return el("span", { class: "embed-glyph", text: "⁘" });
      return el("img", { loading: "lazy", alt: "", src: blobURL(b.thumbCid || b.cid) });
    }
    function makeCell(b) {
      var badge = el("span", { class: "embed-badge" });
      var cell = el("button", { type: "button", class: "embed-cell", title: b.cid },
        [mediaCell(b), badge]);
      cell.dataset.cid = b.cid;
      cell._badge = badge;
      cell.onclick = function () { opts.onPick(b.cid); };
      cells[b.cid] = cell;
      paint(cell);
      return cell;
    }
    function load() {
      ...
      fetch("/embed/blobs?page=" + page).then(function (r) { return r.json(); })
        .then(function (d) {
          ...
          pages = (d && d.pages) || 1;
          var blobs = (d && d.blobs) || [];
          blobs.forEach(function (b) { grid.appendChild(makeCell(b)); });
    ...
    return {
      ...
      prepend: function (cid) {
        if (cells[cid]) return;
        grid.insertBefore(makeCell({ cid: cid }), grid.firstChild);
      },
    };
  }
  ...
  function embedBlob() {
    ...
    m.content.appendChild(el("div", { class: "embed-pane" }, [
      el("div", { class: "embed-top" }, [
        uploadZone(pick, { accept: "image/*,video/*,audio/*",
  ...
  var docHost = document.querySelector("[data-doc-host]");
  if (toolbar || docHost) {
  ...
  function initDocEditor() {
    ...
    // Every .doc-words and .save-note on the page mirrors the editor — the
    // meta band carries one, a save pill can carry another.
    var pageWords = document.querySelectorAll(".doc-words");
    var pageNotes = document.querySelectorAll(".save-note");
    ...
    // ── mode toggle — lives at the head of the toolbar ──
    ...
    // The bar carries only what is not about a selection: the mode toggle
    // and the things you insert. Formatting lives in the selection bubble.
    var toolKids = [];
    if (rich) toolKids.push(el("div", { class: "seg" }, [editBtn, mdBtn]));
    if (cfg.blobs) {
      toolKids.push(sep());
      if (kinds.indexOf("blob") >= 0) {
        toolKids.push(tbtn("Media", "", "Insert an image, video, or audio clip",
    ...
    toolKids.push(sep(), tbHr,
      tbtn("?", "", "Keyboard shortcuts — ⌘/", toggleKeys, false));
    var tools = el("div", { class: "doc-tools" }, toolKids);

    // ── the selection bubble — formatting rides with the selection ──
    // Notion's shape: select text and the scoped controls appear beside it;
    // the link editor is the bubble's second row rather than a bar strapped
    // to the top of the page.
    var bubbleRow = el("div", { class: "doc-row" }, [
      tbBold, tbItal, tbStrike, tbLink, sep(),
      tbH1, tbH2, tbH3, sep(), tbQuote, tbUl, tbOl, tbCode,
    ]);
    var bubble = el("div", { class: "doc-bubble" }, [bubbleRow]);
    bubble.hidden = true;
    // Buttons already preventDefault on mousedown, so pressing them never
    // collapses the selection the bubble is anchored to.
    ...
    // placeBubble anchors the bubble to the live selection: above it when
    // there is room under the sticky bar, below it otherwise, clamped to the
    // viewport. Coordinates are host-relative, so scrolling the page keeps
    // the bubble glued to its text with no repositioning.
    function placeBubble() {
      if (mode !== "rich") { hideBubble(); return; }
      // Focus inside the bubble's own controls is not the selection leaving
      // the document — the link popover's input takes focus by design, which
      // moves the document selection out of the surface and fires
      // selectionchange. Hiding on that would close the popover on the very
      // frame it opens, which is what it used to do.
      if (bubble.contains(document.activeElement)) return;
      var s = window.getSelection();
      if (!s.rangeCount || !surface.contains(s.anchorNode)) { hideBubble(); return; }
      if (s.isCollapsed && pop.hidden) { hideBubble(); return; }
      var range = s.getRangeAt(0);
      // A whole-node selection (a clicked embed island) formats nothing.
      if (range.startContainer === surface) { hideBubble(); return; }
      var rect = range.getBoundingClientRect();
      if (!rect || (rect.width === 0 && rect.height === 0 && s.isCollapsed && pop.hidden)) {
        hideBubble();
        return;
      }
      var hostRect = docHost.getBoundingClientRect();
      bubble.hidden = false;
      var bw = bubble.offsetWidth, bh = bubble.offsetHeight;
      var x = rect.left - hostRect.left + rect.width / 2 - bw / 2;
      var minX = -hostRect.left + 8;
      var maxX = (window.innerWidth - hostRect.left) - bw - 8;
      x = Math.max(minX, Math.min(x, maxX));
      // 44px keeps it clear of the sticky toolbar when the selection is near
      // the top of the viewport.
      var y;
      if (rect.top - bh - 10 > 44) y = rect.top - hostRect.top - bh - 10;
      else y = rect.bottom - hostRect.top + 10;
    ...
    altInput.addEventListener("keydown", function (e) {
      ...
      if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); hideAltPop(); surface.focus({ preventScroll: true }); }
    });
    ...
    // The page itself scrolls the document now, so the page is what autosize
    // must hold still.
    var scroller = document.scrollingElement || document.documentElement;
    form.classList.add("doc-inline");
    // The rail lets the toolbar be wider than the writing column and still
    // center on it — see .doc-tools-rail in the stylesheet.
    docHost.appendChild(el("div", { class: "doc-tools-rail" }, [tools]));
    bubble.appendChild(pop);
    docHost.appendChild(bubble);
    docHost.appendChild(altPop);
    docHost.appendChild(fileInput);
    docHost.appendChild(col);
    ...
    function countWords() {
      var t = wordsText();
      pageWords.forEach(function (n) { n.textContent = t; });
    }
    function autosize() {
      if (mode !== "md") return;
      // Collapsing to "auto" (the only way to measure shrinkage) momentarily
      // empties the scroll container, and the browser clamps its scrollTop to
      // the collapsed height — which read as the page snapping to the bottom
      // on every keystroke. Both writes land in the same frame, so putting
      // the position back is invisible; it just stops the jump.
    ...
    function paintState(text, isDirty, isErr) {
      pageNotes.forEach(function (n) {
        ...
        n.classList.toggle("dirty", !!(isDirty || isErr));
        n.classList.toggle("err", !!isErr);
      });
    }
    // paintSaved is paintState's louder sibling: a confirmation that reads at
    // full strength and carries a link to what was just created.
    function paintSaved(text, href) {
      pageNotes.forEach(function (n) {
        n.textContent = "";
        n.classList.remove("dirty", "err");
        n.classList.add("ok");
        n.appendChild(document.createTextNode(text + " — "));
        n.appendChild(el("a", { href: href, text: "view" }));
      });
    }
    function markDirty() {
      dirty = true;
    ...
    // ── boot ──
    // No opening ceremony: the page is the editor, so the editor is simply
    // on. Rich when the host offers editdoc, markdown otherwise.
    ...
    function save() {
      ...
      fetch(form.action, {
        ...
        // The app may have settled the key on its own terms (a create
        // derives and stamps it). Reflect that in the slug field, or the
        // next save would re-derive a bare one from the title; the input
        // event lets the band relabel its chip.
        var slugField = form.querySelector('input[name="slug"]');
        if (slugField && res.d.slug && slugField.value !== res.d.slug) {
          slugField.value = res.d.slug;
          slugField.dispatchEvent(new Event("input", { bubbles: true }));
        }
        var at = new Date();
        var stamp = at.getHours() + ":" + String(at.getMinutes()).padStart(2, "0");
        // A create is the one save worth more than a timestamp: something now
        // exists that did not before. When the app says where it landed, the
        // note becomes a link to go see it.
        if (res.d.created && res.d.viewURL) {
          paintSaved(cfg.createdLabel || "Posted", res.d.viewURL);
        } else {
          paintState("Saved " + stamp, false);
        }
        // The band resets its publish baseline on this — a save makes the
        // toggled state the saved state.
    ...
    Array.prototype.forEach.call(form.querySelectorAll("[data-doc-save]"),
      function (b) {
    ...
    function restoreSel() {
      ...
      // the snapshot put back. preventScroll everywhere here: the surface is
      // taller than the viewport, and a bare focus() scrolls it "into view",
      // which for a huge element means yanking the page somewhere arbitrary.
      if (s.rangeCount && surface.contains(s.anchorNode)) {
        surface.focus({ preventScroll: true });
        return;
      }
    ...
    function toggleLinkPop() {
      ...
      pop.hidden = false;
      // ⌘K with a bare caret has no bubble yet — placing it now anchors the
      // link editor at the caret. preventScroll: it is already in view.
      placeBubble();
      popInput.focus({ preventScroll: true });
    }
      ...
      Array.prototype.slice.call(files).forEach(function (f) {
        ...
        uploadBlob(f).then(function (cid) {
          ...
          if (isMedia) {
            ...
            // Mirror the server's editable render for each media family, so
            // the embed looks the same now as after a reload.
            var src = escHTML(blobURL(cid));
            if (/^video\//.test(f.type)) {
              fig.innerHTML = '<video class="blob-media standalone" controls ' +
                'preload="metadata" src="' + src + '"></video>';
            } else if (/^audio\//.test(f.type)) {
              fig.innerHTML = '<audio class="blob-media standalone" controls ' +
                'preload="metadata" src="' + src + '"></audio>';
            } else {
